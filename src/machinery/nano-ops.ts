...
import type {Frontier, NANO, NanoAccount, NanoAddress, PrivateKey, PublicKey} from './models';
import {generateWork, getPendingBlocksSimple, loadBlocks, loadFrontiers, processSimple} from "./nano-rpc-fetch-wrapper";
import {rawToNano, signReceiveBlock, signSendBlock} from "./nanocurrency-web-wrapper";
...
export async function loadWalletData(account: NanoAccount): Promise<void> {
  ...
  const frontiers: Map<string, Frontier> | { [key: string]: Frontier; } = await loadFrontiers([account.address]);
  const blocks: { [key: string]: BlockInfo } = await loadBlocks(Object.values(frontiers))
...
async function resolvePendingForAccount(
  ...
  /** TODO: Clean up this */
  const frontierOrPublicKey: Frontier | PublicKey = frontier && frontier !== "" ? frontier : publicKey;
...
export async function sendNano(
  ...
  try {
    ...
    const signed: SignedBlock = signSendBlock(account.privateKey, balance, account.address, toAddress, frontier, amount, workHash);
